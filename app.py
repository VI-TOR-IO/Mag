...
def predict_lstm_multistep(data, model, scaler, seq_length, forecast_length):
    model.eval()
    ...
    pred = scaler.inverse_transform(inverse_input)[:, close_column_index]
    return pred

def backtest_lstm_multistep(data, model, scaler, seq_length, forecast_length, real_dates):
    model.eval()
    close_column_index = 3
    i = len(data) - forecast_length - seq_length - 7
    ...
    for ticker in TICKERS:
        for days in [1, 3, 7, 30]:
            ...
            if os.path.exists(model_path):
                model = LSTMModelMultiStep(input_size=5, forecast_length=days).to(device)
...
def predict():
    ...
    # Прогноз вперед
    future_predictions = predict_lstm_multistep(features, model, scaler, seq_length, forecast_days)
    # Backtest для ошибок
    backtest_predictions, backtest_error, pred_back, real_back, back_dates = backtest_lstm_multistep(
        features, model, scaler, seq_length, forecast_days, real_dates
    )
        ...
        xaxis=dict(
            ...
            rangeselector=dict(
                buttons=list([
                    ...
                    dict(count=30, label='1м', step='day', stepmode='backward'),
                    dict(count=90, label='3м', step='day', stepmode='backward'),
    ...
    # --- Показываем рекомендацию только если акция есть в портфеле пользователя ---
    ...
    if user_id:
        conn = get_db_connection()
        row = conn.execute(
            "SELECT avg_price FROM portfolio WHERE user_id = ? AND ticker = ? AND count > 0",
            (user_id, selected_ticker)
        ).fetchone()
        conn.close()
        if row:
